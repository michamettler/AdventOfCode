    ...
    while (sc.hasNextLine()) {
      ...
      if (differenceX == 0) {
        ...
        if (firstCoordinates[0] < secondCoordinates[0]) {
          int stepper = firstCoordinates[1] > secondCoordinates[1] ? -1 : 1;
          int y = firstCoordinates[1];
          for (int x = firstCoordinates[0]; x <= secondCoordinates[0]; x++) {
            diagram[y][x] = diagram[y][x] != null ? diagram[y][x].add(BigInteger.ONE) : BigInteger.ONE;
            y += stepper;
          }
        } else {
          int stepper = secondCoordinates[1] > firstCoordinates[1] ? -1 : 1;
          int y = secondCoordinates[1];
          for (int x = secondCoordinates[0]; x <= firstCoordinates[0]; x++) {
            diagram[y][x] = diagram[y][x] != null ? diagram[y][x].add(BigInteger.ONE) : BigInteger.ONE;
            y += stepper;
          }
        }
